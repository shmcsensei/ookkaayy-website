...
const headers = await readFile(join(clientRoot, '_headers'), 'utf8');
const proxy = await readFile(join(root, 'proxy.ts'), 'utf8');
assert.match(
  proxy,
  /nonce-\$\{nonce\}/,
  'dynamic CSP must authorize framework scripts with a per-request nonce',
);
for (const requiredHeader of [
  ...
  assert.match(
    headers,
    new RegExp(requiredHeader),
    `${requiredHeader} is missing from the production artifact`,
  );
  assert.match(
    proxy,
    new RegExp(requiredHeader),
    `${requiredHeader} is missing from dynamic responses`,
  );
}
...
for (const file of builtFiles) {
  ...
  if (file.endsWith('.js')) sizes.javascript += info.size;
  if (file.endsWith('.css')) sizes.css += info.size;
}
assert.ok(
...
await writeFile(join(root, 'dist', 'audit-report.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log(
